...
class TestDevelopment(TestCase):
  def test_generateCode(self):
    self.failUnlessEqual(len(self.generateCode(12)), 12+2)

  def test_encodeString(self):
    self.failUnlessEqual(len(self.generateCode(12)), len(self.generateCode(12)))
    self.failUnless(self.generateCode(12).find('-') == 4)
