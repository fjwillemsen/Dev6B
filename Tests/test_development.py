...
class TestDevelopment(TestCase):
  def test_generateCode(self):
    self.failUnlessEqual(len(Development.generateCode(self, 12)), 12+2)

  def test_encodeString(self):
    self.failUnlessEqual(len(Development.generateCode(self, 12)), len(Development.generateCode(12)))
    self.failUnless(Development.generateCode(self, 12).find('-') == 4)

  def test_getDecryptedCode(self):
    code = Development.generateCode(self, 12)
    result = ""
    for i in code:
        if i != '-':
            if int(i) < 5:
